...
public class WebDriverUtil {
	private static Logger logger = MyLogger.getLogger(WebDriverUtil.class);
	...
	public static WebElement getElement(WebDriver driver, String xpath) {
		try {
			...
			// 需要移动的次数
			long times = pageHeight % screenHeight == 0 ? pageHeight
					/ screenHeight : pageHeight / screenHeight + 1;
	...
	public static void wait(WebDriver driver, String xpath) {
		...
		while (true) {
			...
			if (WebDriverUtil.isElementExist(driver, xpath)) {
				break;
			} else {
				time += 100;
	...
	public static String getRootPath(String windowsPath, String linuxPath) {
		...
		if (OSName.toUpperCase().contains("Mac".toUpperCase())) {
			root = System.getProperty("user.home")+ linuxPath;
		}
		return root;
	}
	...
	public static String screenShot(WebDriver driver, String name) {
		 ...
		 */
		String root = getRootPath("d:\\selenium", "/Users/selenium");
