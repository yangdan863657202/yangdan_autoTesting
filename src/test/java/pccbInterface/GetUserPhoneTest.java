 ...
 * Created by yangdan
 */
public class GetUserPhoneTest {
    @Test
    public static void getUserPhoneTest () throws Exception {
        CloseableHttpClient httpclient = HttpClients.createDefault();
        try {
            HttpPost post = new HttpPost("http://api.pccb.com/apitest/get_user_phone/");
            List<NameValuePair> list = new ArrayList<NameValuePair>();
            list.add(new BasicNameValuePair("app_type","1"));
            list.add(new BasicNameValuePair("userid", Des.encryptDES("82044")));
            list.add(new BasicNameValuePair("sign_key", Md5.md5( "82044"+"appGetUserPhone" )));
