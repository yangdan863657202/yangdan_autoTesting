...
public class ModifyPasswordTest {
    @Test
    public static void modifyPasswordTest() throws Exception {
        CloseableHttpClient httpclient = HttpClients.createDefault();
        try {
            HttpPost post = new HttpPost("http://yjf3.pccb.com/api/mobile/modify_password");
            List<NameValuePair> list = new ArrayList<NameValuePair>();
            list.add(new BasicNameValuePair("app_type","1"));
            list.add(new BasicNameValuePair("pwd_type","payPwd"));
            //list.add(new BasicNameValuePair("loginPwd","123456"));
            list.add(new BasicNameValuePair("userid", Des.encryptDES("82044")));
            list.add(new BasicNameValuePair("sign_key", Md5.md5( "1"+"82044"+"pccbYjfUserModifyPassword" )));
